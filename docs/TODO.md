...
| ~~Strength score counts a hold's seconds as reps~~ | **Closed.** Option (b) shipped with `SCORING_VERSION = 2` (b00c1a3): `StrengthIndex.holdScore` converts seconds to rep-equivalents at 5 s each, every caller routes holds through it, and `rescoreStrengthScores` restated stored history once on the version bump. Pinned by `StrengthIndexTest` ("a hold scores on its seconds, not as that many reps") | Done |
...
| ~~Idle accrual is unbounded~~ | **Closed, option (c).** `Idle.MAX_EFFECTIVE_HOURS = 72`: the taper and the 10% floor are kept, but one absence never pays more than three full days of work. A rest week still pays 60 effective hours; the cap bites from day 12; a year away pays 72, not 919. Already-banked essence is not restated. Pinned by `IdleTest` ("no absence pays more than the cap"), mutation-checked | Done |
...
| ~~Off-device backup vs. on-device privacy~~ | **Closed.** BACK UP NOW uploads the training archive to `cloud_archives` (0013) and RESTORE brings it back on a new phone. The upload omits private notes, readings, measurements, height, sex and Health Connect days, and a restore keeps the device's own copies (`ExportRoundTripTest`, mutation-checked both ways) | Done |
