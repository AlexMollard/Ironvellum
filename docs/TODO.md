...
| Portrait lock | Landscape gave the quest panel ~400dp of height and it collapsed to nothing — no card, no button, unreachable by scrolling since Court does not scroll by design. Shipped: `screenOrientation="portrait"`. (a) keep the lock; (b) build a landscape layout for Court (two columns); (c) make Court scroll in landscape only | **(a)**, shipped. Every screen is a tall column and the product rule is one screen; a landscape variant is a design project, not a fix. Android 16 ignores the lock on large screens, where the height makes it moot |
