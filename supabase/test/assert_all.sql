...
--
-- The probes next to this file are for reading: they print values a human
...
--   psql -f supabase/migrations/0001_init.sql ... (every migration, in order)
...
-- Runs a statement as a hunter and reports whether it was refused. Used for
-- the checks whose whole point is that an operation is denied.
create or replace function refused_as(uid uuid, stmt text)
...
-- Views: checked separately so the catalogue join stays readable.
do $$
declare
    leaky text;
begin
...
begin
    ...
    perform assert_true(
        refused_as(cass, format('select is_friend(%L, %L)', ayla, cass)),
        'is_friend() is still callable by authenticated (the friendship oracle is open)'
    );

    -- 0009 F2: but can_view must remain callable, or profiles_read breaks.
    perform assert_true(
        not refused_as(cass, format('select can_view(%L)', ayla)),
        'can_view() is no longer callable, which breaks every profile read'
    );

    -- 0009 F3: free text that a feed row re-serves must be bounded.
    perform assert_true(
        refused_as(ayla, format(
            'insert into session_sets (session_id, exercise_name, set_index, reps) values (%L, repeat(''x'', 5000), 90, 1)',
            'a5511111-0000-4000-8000-000000000001')),
        'session_sets.exercise_name accepts unbounded text (one row floods every feed)'
    );

    -- 0009 F4: a future session pins itself to the top of every feed.
    perform assert_true(
        refused_as(ayla, format(
    ...
    -- ...and must not leak the profile behind the name.
    ...
    perform assert_true(
        refused_as(ayla, format('update profiles set total_xp = 9223372036854775807 where id = %L', ayla)),
        'total_xp is directly writable: one PATCH tops the leaderboard'
    );
    perform assert_true(
        refused_as(ayla, format('update profiles set level = 99999 where id = %L', ayla)),
        'level is directly writable'
    );

    -- 0011: the columns a hunter genuinely owns must still be writable.
    perform assert_true(
        not refused_as(ayla, format('update profiles set display_name = ''Ayla'', visibility = ''friends'' where id = %L', ayla)),
        'a hunter can no longer rename themselves'
    );

    -- 0011: the RPC bounds a claim and derives what it can.
    ...
    -- 0011: level always follows the XP it is justified by.
...
drop function if exists refused_as(uuid, text);
