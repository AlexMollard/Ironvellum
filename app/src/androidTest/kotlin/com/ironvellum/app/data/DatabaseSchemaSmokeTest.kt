...
class DatabaseSchemaSmokeTest {
    ...
    fun daoRoundTripsAgainstRealSchema() = runTest {
        ...
        db.profileDao().upsert(
            ProfileEntity(name = "Lifter", totalXp = 10, currentTitleId = null, heightCm = 180.0, sex = "MALE"),
        )
        val profile = db.profileDao().get()
        assertEquals("Lifter", profile?.name)
    ...
    fun databaseOpensAtDeclaredVersion() {
        val opened = Room.databaseBuilder(context, IronvellumDatabase::class.java, TEST_DB)
            .addMigrations(*IronvellumDatabase.MIGRATIONS)
            .build()
        opened.openHelper.writableDatabase.version.let { version ->
            assertEquals(IronvellumDatabase.VERSION, version)
        }
        opened.close()
    }
    ...
    private companion object {
        ...
        const val TEST_DB = "ironvellum-schema-smoke-test.db"
    }
}
