...
/**
 * Full-bleed animated atmosphere behind the Shadow page. Purely decorative:
 ...
 */
@Composable
fun ShadowBackdrop(shadows: Int, active: Boolean, modifier: Modifier = Modifier) {
    // One slow driving float, reversed so nothing ever snaps.
    val breath = rememberInfiniteTransition(label = "shadowBackdrop")
    ...
    // Army growth thickens the field: 10 particles at 0 shadows, ~40 at 400+.
    val visibleCount = (10 + (shadows.coerceAtLeast(0)) * 30 / 400).coerceIn(10, 40)
    // Idle army: damped life, never frozen to nothing.
    ...
    Canvas(modifier = modifier) {
        ...
        drawCircle(
            brush = Brush.radialGradient(
                colors = listOf(
                    MonarchColors.EmeraldBright.copy(alpha = glowAlpha),
                    MonarchColors.Emerald.copy(alpha = glowAlpha * 0.5f),
                    Color.Transparent,
                ),
        ...
        drawRect(
            brush = Brush.verticalGradient(
                colors = listOf(Color.Transparent, MonarchColors.Abyss.copy(alpha = 0.9f)),
                startY = h * 0.78f,
                endY = h,
            ),
