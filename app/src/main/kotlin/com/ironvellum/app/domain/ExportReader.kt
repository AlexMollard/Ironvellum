...
object ExportReader {
    ...
    /**
     * The top crest frame's catalogue id was "monarch" before the rename; an
     * archive written by an older build still carries it. Mapping it here is
     * what stops a restored crest from pointing at a frame the catalogue no
     * longer answers to, which would silently draw nothing.
     */
    private fun legacyFrameId(id: String?): String? =
        if (id == "monarch") "masterwork" else id
